...
-- A Graph_Link which keeps its links in a dynamically allocated linked
-- list.
...
--$END STORAGE_POOL
package Asgc.Graph.Links.DynamicMANAGED is
...
private
   ...
   type Dynamic_Graph_Edge;
   type Dynamic_Graph_Edge_Ptr is access all Dynamic_Graph_Edge;
--$START STORAGE_POOL
   for Dynamic_Graph_Edge_Ptr'Storage_Pool use My_Pool;
--$END STORAGE_POOL
   type Dynamic_Graph_Edge is record
      Val      : Link_Type;
      Contents : Link_Contained_Type;
      Next     : Dynamic_Graph_Edge_Ptr := null;
   end record;

   type Graph_Link is new Graph_Link_Parent with record
      Edges      : Dynamic_Graph_Edge_Ptr := null;
      Link_Count : Natural                := 0;
   end record;

   type Graph_Link_It is new Graph_Link_It_Parent with record
      Robj   : Graph_Link_Class;
      Prev   : Dynamic_Graph_Edge_Ptr   := null;
      Curr   : Dynamic_Graph_Edge_Ptr   := null;
   end record;
