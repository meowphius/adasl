...
------------------------------------------------------------------------
-- A Graph_Link that keeps its links inside a dynamically allocated
-- array that will be reallocated as necessary for growth.
generic
...
private
   ...
   type Edge_Array is array (Positive range <>) of Link;
   type Edge_Array_Ptr is access all Edge_Array;
--$START STORAGE_POOL
   for Edge_Array_Ptr'Storage_Pool use My_Pool;
--$END STORAGE_POOL

   type Graph_Link is new Graph_Link_Parent with record
      -- We start of with a null edge array.  This makes copying the array
      -- much easier, since it won't allocate and possibly leak anything if
      -- assigned over.  Using a controlled type might make it even easier,
      -- but that makes things slower, too.
      Edges    : Edge_Array_Ptr := null;
      Size     : Natural        := 0;

      -- How many times is the graph link begin used?
      Refcount : Natural        := 0;
   end record;
